...
rfnoc_fft::sptr rfnoc_fft::make(rfnoc_graph::sptr graph,
                                      ...
                                      const int instance)
{
    return gnuradio::make_block_sptr<rfnoc_fft_impl>(rfnoc_block::make_block_ref(
        graph, block_args, "FFT", device_select, instance));
}


rfnoc_fft_impl::rfnoc_fft_impl(::uhd::rfnoc::noc_block_base::sptr block_ref)
    : rfnoc_block(block_ref),
      d_fft_ref(get_block_ref<::uhd::rfnoc::fft_block_control>())
{
}
...
void rfnoc_fft_impl::set_direction(const std::string& direction)
{
    if (direction == "REVERSE") {
        ...
        throw std::runtime_error(fmt::format(
            "FFT direction ({:s}) is not [REVERSE, FORWARD]", direction));
    }
}
...
std::string rfnoc_fft_impl::get_direction_string() const
{
    ...
    default:
        throw std::runtime_error(fmt::format(
            "FFT direction enumeration ({:d}) is not [REVERSE, FORWARD]", 
...
std::string rfnoc_fft_impl::get_shift_config_string() const
{
    ...
    default:
        throw std::runtime_error(fmt::format(
            "FFT shift configuration enumeration ({:d}) is not [NORMAL, REVERSE, NATURAL, BIT_REVERSE]", 
...
std::string rfnoc_fft_impl::get_magnitude_string() const
{
    ...
    default:
        throw std::runtime_error(fmt::format(
            "FFT magnitude enumeration ({:d}) is not [COMPLEX, MAGNITUDE, MAGNITUDE_SQUARED]", 
